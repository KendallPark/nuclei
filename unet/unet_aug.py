...
def train(X_train, Y_train, X_val, Y_val, model, filename):
    ...
    results = model.fit_generator(transform.make_double_generator(X_train, Y_train, 8, augmentor), epochs=100, 
                              steps_per_epoch=76,
                              validation_data=transform.make_double_generator(X_train, Y_train, 8, augmentor),
                              workers=8,
                              validation_steps=9,
                              callbacks=[checkpointer])
    # results = model.fit(X_train, Y_train, validation_data=(X_val, Y_val), batch_size=8, epochs=150, callbacks=[checkpointer])
